<template>
  <div style="width: 100%; position: relative">
    ...
    <div v-if="isLoading" class="loading-bar" style="
        position: absolute;
        z-index: 10;
        top: 50%;
        left: 50%;
...
const satelliteGroups = [
  ...
  {
    name: "Russian ASAT Test",
    url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=1982-092&FORMAT=json",
  },
  {
    name: "Chinese ASAT Test",
    url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=1999-025&FORMAT=json",
  },
  {
  ...
  {
    name: "Search & Rescue",
    url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=sarsat&FORMAT=json",
  },
  {
    name: "Planet",
    url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=planet&FORMAT=json",
  },
  {
  ...
  {
    name: "Other",
    url: "https://celestrak.org/NORAD/elements/gp.php?GROUP=other&FORMAT=json",
  },
];
...
const loadSatelliteData = async (url: String) => {
  ...
  const objectsToRemove =
    scene?.children.filter((object) => object.userData.type === "satellite") ??
    [];
...
const satellitePosition = (satellite: any) => {
  ...
  const semiMajorAxis =
    Math.pow(
      (6.6743e-11 * 5.97219e24) /
      (4 * Math.PI ** 2 * satellite.MEAN_MOTION ** 2),
      1 / 3,
    ) / 1000;
  ...
  const spherical = new THREE.Spherical(
    ...
    Math.PI / 2 -
    Math.asin(
      Math.sin(inclination) * Math.sin(argOfPericenter + Math.cos(E)),
    ),
    raOfAscNode +
    Math.atan2(
...
const animate = () => {
  if (mounted.value) {
    ...
    for (const object of scene?.children.filter(
      (object) => object.userData.type === "satellite",
    ) ?? []) {
